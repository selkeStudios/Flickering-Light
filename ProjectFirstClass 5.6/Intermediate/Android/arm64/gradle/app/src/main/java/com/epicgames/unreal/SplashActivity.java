...
public class SplashActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (!isTaskRoot())
		{
			...
			// see if foregrounded by launcher main action (like from home screen icon)
			if (intent.hasCategory(Intent.CATEGORY_LAUNCHER) && intent.getAction() != null && intent.getAction().equals(Intent.ACTION_MAIN))
			{
				super.onCreate(savedInstanceState);

				// leave previous top activity at top instead of forcing GameActivity
				finish();
				return;
			}
		}
		...
		com.epicgames.unreal.GameApplication.startLoadLibraries();
		...
		// pass the data if available
		Uri intentData = intentFromActivity.getData();
		if (intentData != null)
		{
			GameActivityIntent.setData(intentData);
		}
		...
		LinkedList<String> ungrantedPermissions = getUngrantedPermissions(this, getDangerousPermissions(pm, packageName), permissionsRequiredAtStart);
	...
	private String[] filterRequiredPermissions(String permissions)
	{
		...
		LinkedList<String> keptPermissions = new LinkedList<>();
	...
	public LinkedList<String> getUngrantedPermissions(Context context, LinkedList<String> dangerousPermissions, String[] requiredPermissions)
	{
	...
	public LinkedList<String> getDangerousPermissions(PackageManager pm, String packageName)
	{
		int targetSdkVersion = 0;
		LinkedList<String> dangerousPermissions = new LinkedList<>();
		try 
		{
			...
			public void run()
			{
				...
				if (!dialogSettings)
				{
					dialog.setPositiveButton(getResourceStringOrDefault(packageName, "PERM_OK", "OK"), new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int id) {
							dialog.dismiss();
							LinkedList<String> ungrantedPermissions = getUngrantedPermissions(SplashActivity.this, getDangerousPermissions(pm, packageName), permissionsRequiredAtStart);
							if (ungrantedPermissions.size() > 0)
							{
